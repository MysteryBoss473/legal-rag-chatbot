...
class ChromaCloudClient:
    ...
    def _use_cloud(self) -> bool:
        """Verifie si les credentials ChromaDB Cloud sont tous fournis."""
        return all([
            self.settings.chroma_api_key and self.settings.chroma_api_key.strip(),
            self.settings.chroma_tenant and self.settings.chroma_tenant.strip(),
            self.settings.chroma_database and self.settings.chroma_database.strip(),
        ])
    ...
    def _get_client(self):
        ...
        if self._use_cloud():
            ...
            logger.info("Credentials ChromaDB Cloud non fournis, utilisation du stockage local")
            self._client = chromadb.PersistentClient(
                path="./chroma_data",
                settings=ChromaSettings(anonymized_telemetry=False)
            )
